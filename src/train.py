...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if valid_loss_min is None or (
            ...
            # Check if data is a tensor or dictionary of tensors
            if isinstance(model.state_dict(), dict) and all(isinstance(v, torch.Tensor) for v in model.state_dict().values()):
                torch.save(model.state_dict(), save_path)
            else:
                print(f"Error: Data type not supported for saving. Type: {type(model.state_dict())}")
